...
import type { ReceiptLineItem } from '@web/components/patterns/themed/Receipt';
...
export function OrderConfirmation({
  ...
  // Map order items to Receipt's ReceiptLineItem shape.
  // unitPrice stored as snapshot price (already per-unit).
  ...
  // Per-item renderer passed to Receipt so cancel buttons, notes, and badges
  // are preserved inside the themed receipt layout.
  const renderItem = (_lineItem: ReceiptLineItem, idx: number) => {
        ...
        <div className="flex items-center gap-2 shrink-0 ml-2">
          ...
          {isModifiable && itemStatus === 'active' && (
            <button
              ...
              aria-label={`${t('Request cancellation for')} ${item.name}`}
            >
              <X className="h-5 w-5" />
            </button>
          )}
  ...
  return (
    <div className="p-4 flex flex-col gap-5 pb-8">
      ...
      {/* Status tracker — replaced StatusTimeline with themed OrderTracker */}
      {!isCancelled && (
        <OrderTracker
          theme={themeId}
          status={order.status}
      ...
      {/* Order items + totals via themed Receipt */}
