...
For workflow migration, architecture changes, or second-project setup, also read `.codex/workflow/principles.md`, `.codex/workflow/capabilities.md`, `.codex/workflow/system/README.md`, `.codex/workflow/project/README.md`, and `.codex/workflow/templates/project-bootstrap.md` before editing workflow scripts, policy, hooks, or project instructions.
...
- Treat `.codex/workflow/system/scripts/workflow-kernel.mjs` as the reusable deterministic workflow kernel, `.codex/workflow/system/scripts/workflow-engine.mjs` as the reusable profile/policy loader, and `.codex/scripts/nexus-workflow.mjs` as the Nexus wrapper shim. Hooks, package scripts, server checks, and future CI should call the wrapper; LLM judgment should be recorded as review/verify/audit/routing/pattern evidence for the kernel to validate.
- Put project-specific workflow facts in `.codex/workflow/profile.json`, `.codex/workflow/policy/*.json`, `.codex/knowledge/*`, and exact-file adapter sources before hardcoding them in scripts. `.codex/workflow/policy/adapters.json` owns the adapter target/source-owner map; exact-file adapters use `.codex/workflow/project/adapters/*`, while package workflow scripts use `.codex/workflow/policy/gates.json` `gates.packageScripts`.
- Use `.codex/workflow/policy/files.json` `inventory.roleTaxonomy` to classify workflow paths before editing or porting them. It identifies system code, project adapter sources, project data/policy, workflow docs, append-only evidence, generated artifacts, mutable cache/runtime telemetry, and historical research.
