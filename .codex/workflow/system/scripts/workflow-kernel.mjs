...
import { appendFileSync, copyFileSync, existsSync, mkdtempSync, mkdirSync, readFileSync, readdirSync, rmSync, statSync, writeFileSync } from 'node:fs';
...
const KERNEL_SCRIPT = fileURLToPath(import.meta.url);
...
const ZOO_GUIDE_DIR = optionalProfileProjectPath('zooGuideDir', '.codex/dashboard/zoo');
const ZOO_GUIDE_MANIFEST = optionalProfileProjectPath('zooGuideManifest', '.codex/dashboard/zoo/manifest.json');
...
const WORKFLOW_WRAPPER_REL = projectRel(WORKFLOW_WRAPPER);
const KERNEL_SCRIPT_REL = projectRel(KERNEL_SCRIPT);
...
const PUBLIC_GUIDE_URL = optionalPolicyString('deployment', 'publicGuideUrl');
const PUBLIC_ZOO_GUIDE_URL = optionalPolicyString('deployment', 'visualZooGuideUrl');
const PUBLIC_GUIDE_VERSION = requiredPolicyString('guide', 'version');
const DESIGN_ACTIVE = designCapabilityActive();
const ZOO_VISUAL_GUIDE_VERSION = DESIGN_ACTIVE ? requiredPolicyString('design', 'zooVisualGuideVersion') : optionalPolicyString('design', 'zooVisualGuideVersion', 'design-zoo-visual-guide/v1');
...
const DESIGN_ROUTE = DESIGN_ROUTE_VALUE ? (DESIGN_ROUTE_VALUE.startsWith('/') ? DESIGN_ROUTE_VALUE : `/${DESIGN_ROUTE_VALUE}`) : '';
const PRODUCTION_DESIGN_PATH = DESIGN_ACTIVE ? requiredPolicyString('design', 'productionDesignPath') : optionalPolicyString('design', 'productionDesignPath', '');
const DESIGN_REGISTRY_PATH = optionalPolicyString('design', 'registryPath', '');
const DESIGN_ZOO_ROUTE_PATH = optionalPolicyString('design', 'zooRoutePath', '');
const DESIGN_THEME_MATRIX_PATH = optionalPolicyString('design', 'themeMatrixPath', '');
const DESIGN_THEME_DIRECTORY_PATH = optionalPolicyString('design', 'themeDirectoryPath', '');
...
function optionalProfileProjectPath(key, fallbackRel) {
  const value = PROFILE.paths?.[key];
  return resolveProjectPath(typeof value === 'string' && value ? value : fallbackRel);
}
...
function optionalPolicyString(sectionName, key, fallback = '') {
  const value = POLICY[sectionName]?.[key];
  return typeof value === 'string' && value.length ? value : fallback;
}

function optionalPolicyArray(sectionName, key, fallback = []) {
  const value = POLICY[sectionName]?.[key];
  return Array.isArray(value) ? value : fallback;
}

function capabilityState(id, policy = POLICY) {
  const entry = (policy.portability?.capabilities || []).find((item) => item?.id === id);
  return entry?.state || 'unsupported';
}

function capabilityIsActive(id, policy = POLICY) {
  return ['required', 'active-optional', 'active-wrapper-coupled'].includes(capabilityState(id, policy));
}

function capabilityIsEnabled(id, policy = POLICY) {
  return capabilityState(id, policy) !== 'unsupported';
}

function capabilitySummary(policy = POLICY) {
  return (policy.portability?.capabilities || []).map((entry) => ({
    id: entry.id,
    state: entry.state,
    active: capabilityIsActive(entry.id, policy),
    owner: entry.owner || '',
    emptyProjectRule: entry.emptyProjectRule || '',
  }));
}

function designCapabilityActive(policy = POLICY) {
  return capabilityIsActive('design-system-zoo-gym', policy);
}

function deploymentCapabilityActive(policy = POLICY) {
  return capabilityIsActive('deployment-validation', policy);
}

function guideCapabilityActive(policy = POLICY) {
  return capabilityIsActive('generated-guide', policy);
}

function dependencyAuditCapabilityActive(policy = POLICY) {
  return capabilityIsActive('dependency-audit-baseline', policy);
}

function ciWorkflowCapabilityActive(policy = POLICY) {
  return capabilityIsActive('ci-workflow-gates', policy);
}

function projectDisplayName() {
  return PROFILE.displayName || PROFILE.projectId || 'Codex';
}

function workflowMessagePrefix() {
  return `${projectDisplayName()} workflow`;
}

function ciWorkflowPath(policy = POLICY) {
  return policy.gates?.ciWorkflowPath || '.github/workflows/workflow-gates.yml';
}
...
function helpTextForCommand(commandName = '') {
  ...
  const usage = {
    ...
    'portability-check': `${script} portability-check [--keep]`,
...
function zooVisualSourceHash() {
  if (!designCapabilityActive()) return 'disabled';
...
function publicZooManifest(manifest = {}) {
  return {
    schema: manifest.schema || POLICY.design?.zooVisualManifestSchema || 'codex-design-zoo-visual-manifest/v1',
...
function guideViewContractProblems(html, label = 'guide', options = {}) {
  ...
  const headings = Array.isArray(contract.timelineCategoryHeadings)
    ? contract.timelineCategoryHeadings
    : ['Workflow', 'Validation', 'Knowledge', 'Agent Routing', 'Deployment'];
...
function commandDashboard(args = {}) {
  ...
  const designActive = designCapabilityActive();
  const registry = designActive ? loadJson(resolveProjectPath(DESIGN_REGISTRY_PATH), {
    primitives: [],
    patterns: [],
  }) : { primitives: [], patterns: [] };
...
<body>
  ...
  <div class="layout">
    <nav>
      ...
      ${designActive ? '<a href="#zoo">Design Zoo/Gym</a>' : ''}
    ...
    <main>
      ...
      ${designActive ? `<section id="zoo" class="panel">
...
function commandPublicGuide(args = {}) {
  ...
  const designActive = designCapabilityActive();
  const registry = designActive ? loadJson(resolveProjectPath(DESIGN_REGISTRY_PATH), { primitives: [], patterns: [] }) : { primitives: [], patterns: [] };
  const zooEntries = [...(registry.primitives || []), ...(registry.patterns || [])];
  const zooLinked = zooEntries.filter((entry) => entry.zooRoute).length;
  const themes = designActive ? designThemeNames(registry) : [];
  ...
  const designSystemGraph = designActive ? guidePolicyGraph('designSystem') : null;
  const designFlowGraph = designActive ? guidePolicyGraph('designFlow') : null;
...
<body>
  ...
  <main>
    ...
    ${designActive ? `<section>
      ...
      <h2>Optional Capabilities And Docs</h2>
      <div class="grid">
        ${capabilitySummary().filter((capability) => !capability.active).map((capability) => `<div class="card"><strong>${publicHtml(capability.id)}</strong><p class="meta">${publicHtml(capability.state)}</p><p>${publicHtml(capability.emptyProjectRule || '')}</p></div>`).join('\n')}
      </div>
...
function commandZooVisualGuide(args = {}) {
  if (!designCapabilityActive()) {
    if (!args.quiet) console.log('Zoo visual guide skipped: design-system-zoo-gym capability is not active.');
    return;
  }
...
function ciWorkflowContractProblems(options = {}) {
  if (!ciWorkflowCapabilityActive(options.policy || POLICY)) return [];
  ...
  const ciText = options.text ?? readText(options.path || ciWorkflowPath(options.policy || POLICY));
...
function profileContractProblems(profile = PROFILE) {
  ...
  if (guideCapabilityActive()) requiredPaths.push('dashboardDir', 'guideBrowserFinalArtifacts');
  if (designCapabilityActive()) requiredPaths.push('zooGuideDir', 'zooGuideManifest');
  if (capabilityIsActive('model-routing')) requiredPaths.push('routingScenarios');
  ...
  const requiredEnv = ['publicWorkflowUrl'];
  if (designCapabilityActive()) requiredEnv.push('webUrl', 'zooCaptureTheme', 'zooCaptureMode');
...
function policyProblems() {
  ...
  const requiredGraphKeys = ['repository', 'webApp', 'workflow'];
  if (designCapabilityActive()) requiredGraphKeys.push('designSystem', 'designFlow');
...
function commandCapabilityCheck({ quiet = false } = {}) {
  const capabilities = capabilitySummary();
  const problems = [];
  const states = new Set(POLICY.portability?.states || []);
  for (const capability of capabilities) {
    if (!capability.id) problems.push('portability capability entry is missing id.');
    if (!states.has(capability.state)) problems.push(`portability capability ${capability.id || '(missing)'} has unsupported state ${capability.state || '(missing)'}.`);
    if (capability.active && !capability.owner) problems.push(`active capability ${capability.id} must declare an owner.`);
    if (!capability.emptyProjectRule) problems.push(`capability ${capability.id || '(missing)'} must declare emptyProjectRule.`);
  }
  if (!capabilities.some((capability) => capability.id === 'core-workflow-ladder' && capability.active)) {
    problems.push('core-workflow-ladder capability must be active.');
  }
  if (!capabilities.some((capability) => capability.id === 'patch-review-verify-audit' && capability.active)) {
    problems.push('patch-review-verify-audit capability must be active.');
  }
  if (!quiet) {
    console.log(`workflow capabilities: ${capabilities.length}`);
    for (const capability of capabilities) {
      console.log(`- ${capability.id}: ${capability.state}${capability.active ? ' (active)' : ' (inactive)'}`);
    }
    console.log(`workflow capability problems: ${problems.length}`);
    for (const problem of problems) console.log(`- ${problem}`);
  }
  return problems.length === 0;
}

const PORTABLE_EMPTY_FIXTURE = join(dirname(KERNEL_SCRIPT), '..', 'fixtures', 'portable-empty');

function copyPortableFixtureTree(source, target) {
  ensureDir(target);
  for (const entry of readdirSync(source, { withFileTypes: true })) {
    const sourcePath = join(source, entry.name);
    const targetPath = join(target, entry.name);
    if (entry.isDirectory()) {
      copyPortableFixtureTree(sourcePath, targetPath);
      continue;
    }
...
function writePortableProject(root) {
  if (!existsSync(PORTABLE_EMPTY_FIXTURE)) {
    throw new Error('portable empty-project fixture is missing: ' + PORTABLE_EMPTY_FIXTURE);
  }
  copyPortableFixtureTree(PORTABLE_EMPTY_FIXTURE, root);
  ensureDir(join(root, '.codex', 'workflow', 'system', 'scripts'));
  writeFileSync(join(root, '.codex', 'workflow', 'system', 'scripts', 'workflow-kernel.mjs'), readFileSync(KERNEL_SCRIPT, 'utf8'));
  writeFileSync(join(root, '.codex', 'workflow', 'system', 'scripts', 'workflow-engine.mjs'), readFileSync(join(dirname(KERNEL_SCRIPT), 'workflow-engine.mjs'), 'utf8'));
  writeFileSync(join(root, '.codex', 'workflow', 'system', 'scripts', 'run-hook.mjs'), readFileSync(join(dirname(KERNEL_SCRIPT), 'run-hook.mjs'), 'utf8'));
  copyPortableFixtureTree(PORTABLE_EMPTY_FIXTURE, join(root, '.codex', 'workflow', 'system', 'fixtures', 'portable-empty'));
  return { root, wrapperRel: '.codex/scripts/portable-workflow.mjs' };
}

function collectTextFiles(root, options = {}) {
  const maxBytes = Number(options.maxBytes || 1024 * 1024);
  const allowedExtensions = new Set(options.extensions || ['.mjs', '.js', '.json', '.md', '.toml', '.yaml', '.yml', '.txt']);
  const files = [];
  const visit = (dir) => {
...
function portableProjectLeakLines(projectRoot, forbiddenStrings) {
  const leaks = [];
  for (const file of collectTextFiles(projectRoot)) {
    const text = readFileSync(file, 'utf8');
    const lines = text.split(/\r?\n/);
    for (let index = 0; index < lines.length; index++) {
      const line = lines[index];
      if (!forbiddenStrings.some((token) => token && line.includes(token))) continue;
      leaks.push(`${relative(projectRoot, file).replaceAll('\\', '/')}:${index + 1}:${line.trim()}`);
    }
  }
  return leaks;
}

function sourceProjectLeakStrings() {
  return Array.from(new Set([
    ...
    ['ten', 'ant'].join(''),
    ['rest', 'aurant'].join(''),
    ...asArray(POLICY.portability?.sourceProjectLeakStrings).map(String),
  ].filter(Boolean)));
}

function commandPortabilityCheck(args = {}) {
  const root = mkdtempSync(join(tmpdir(), 'codex-workflow-portability-'));
  const keep = Boolean(args.keep);
  const commands = [
    { label: 'status', args: ['status'] },
    { label: 'capability-check', args: ['capability-check'] },
    { label: 'policy-check', args: ['policy-check'] },
    ...
    { label: 'guide-check', args: ['guide-check'] },
    { label: 'self-test', args: ['self-test'] },
  ];
  const results = [];
  try {
    const project = writePortableProject(root);
    const wrapper = join(root, project.wrapperRel);
    ...
    const leaked = portableProjectLeakLines(root, sourceProjectLeakStrings());
    const ok = results.every((result) => result.status === 0 && !result.timedOut) && leaked.length === 0;
    if (!args.quiet) {
      console.log(`portable empty-project root: ${root}`);
      for (const result of results) {
        console.log(`${result.status === 0 && !result.timedOut ? 'PASS' : 'FAIL'} ${result.command}`);
        if (result.status !== 0 || result.timedOut) {
          if (result.stdout) console.log(result.stdout.trim());
          if (result.stderr) console.error(result.stderr.trim());
        }
      }
      console.log(`${leaked.length === 0 ? 'PASS' : 'FAIL'} portable project has no source-project app literals`);
      if (leaked.length) console.log(leaked.slice(0, 50).join('\n'));
    }
    return ok;
  } finally {
...
function commandDependencyAuditCheck({ quiet = false } = {}) {
  if (!dependencyAuditCapabilityActive()) {
    if (!quiet) console.log('dependency audit skipped: dependency-audit-baseline capability is not active.');
    return true;
  }
...
function commandProductionZooBundleCheck({ quiet = false } = {}) {
  if (!designCapabilityActive()) {
    if (!quiet) console.log('production Zoo bundle check skipped: design-system-zoo-gym capability is not active.');
    return true;
  }
...
function commandRunCommand(rawArgs = [], options = {}) {
  ...
  if (!args.commandArgs.length) {
    if (options.error !== false) console.error(`run-command requires a command after --, for example: node ${WORKFLOW_WRAPPER_REL} run-command --id status -- npm run workflow:status`);
    return 2;
  }
  const telemetryFile = options.telemetryFile || COMMAND_RUNS_FILE;
...
function printHealthDetails() {
  ...
  if (guideCapabilityActive() && !commandGuideCheck({ quiet: true })) {
    const problems = [];
  ...
  if (guideCapabilityActive()) printed = printProblemDetails('guide browser', guideBrowserProblems()) || printed;
  if (designCapabilityActive()) printed = printProblemDetails('zoo registry', zooRegistryProblems()) || printed;
  if (designCapabilityActive()) printed = printProblemDetails('zoo visual guide', zooVisualGuideProblems()) || printed;
...
function commandStatus({ health = false } = {}) {
  ...
  const guideOk = health ? (!guideCapabilityActive() || commandGuideCheck({ quiet: true })) : null;
  const guideBrowserOk = health ? (!guideCapabilityActive() || commandGuideBrowserCheck({ quiet: true })) : null;
  const zooVisualOk = health ? (!designCapabilityActive() || commandZooVisualGuideCheck({ quiet: true })) : null;
...
function deploymentProblems() {
  if (!deploymentCapabilityActive()) return [];
  const branch = branchEvidenceInfo();
  const problems = [];
  if (!branch.hash) {
...
function commandDeploymentCheck({ quiet = false } = {}) {
  if (!deploymentCapabilityActive()) {
    if (!quiet) console.log('deployment check skipped: deployment-validation capability is not active.');
    return true;
  }
...
function workflowSelfTestChecks() {
  ...
  const leadWorker = leadWorkerId() || 'codex-lead';
  const sparkWorker = defaultWorkerForRoute('spark') || 'spark_worker';
  const strongWorker = defaultWorkerForRoute('strong') || escalationWorkerId() || 'strong_worker';
  const researchWorker = defaultWorkerForRoute('research') || 'research_worker';
  const reviewWorker = defaultWorkerForRoute('review') || 'review_worker';
  const branchFixtureFiles = [WORKFLOW_WRAPPER_REL];
  const policyFixtures = policyClassifierTestCases();
  const fixtureFile = (predicate, fallback = WORKFLOW_WRAPPER_REL) => policyFixtures.find(predicate)?.file || fallback;
  const substantiveFixture = fixtureFile((testCase) => testCase.substantive && !String(testCase.file || '').startsWith('.codex/'), WORKFLOW_WRAPPER_REL);
  const verificationFixture = fixtureFile((testCase) => testCase.verification, substantiveFixture);
  const auditFixture = fixtureFile((testCase) => testCase.audit, substantiveFixture);
  ...
  const withProjectTempArtifact = (content, fn) => {
    ensureDir(GUIDE_BROWSER_FINAL_ARTIFACTS_DIR);
    const dir = mkdtempSync(join(GUIDE_BROWSER_FINAL_ARTIFACTS_DIR, 'artifact-proof-'));
  ...
  const fixtureHandoverCheck = (name, text, expectedOk) => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-workflow-self-test-'));
    ensureDir(dir);
    const path = join(dir, `${slug(name)}.md`);
    writeFileSync(path, text);
    const script = process.argv[1] ? resolve(process.argv[1]) : WORKFLOW_WRAPPER;
  ...
  add('visual zoo guide artifacts are non-substantive but trigger visual gates', !designCapabilityActive() || substantiveFiles(['.codex/dashboard/zoo/index.html', '.codex/dashboard/zoo/assets/button.jpg']).length === 0
  ...
  add('workflow wrapper and system kernel are required workflow manifest', requiredWorkflowFiles().includes(WORKFLOW_WRAPPER_REL) && requiredWorkflowFiles().includes(KERNEL_SCRIPT_REL));
  ...
  add('portable empty-project fixture is required workflow manifest', requiredWorkflowFiles().includes('.codex/workflow/system/fixtures/portable-empty/'));
  ...
  add('review kind policy classifies design source changes', !designCapabilityActive() || requiredReviewKinds([designTokenSourceFile()]).includes('design'));
  add('review kind policy classifies design policy changes', !designCapabilityActive() || requiredReviewKinds(['.codex/workflow/policy/design.json']).includes('design'));
  ...
  add('branch review policy has policy-owned pattern-sensitive fixtures', policyClassifierTestCases()
    .filter((testCase) => testCase.substantive)
    .some((testCase) => requiredBranchReviewKinds([testCase.file]).includes('pattern')));
  add('audit classifier includes policy-owned audit fixtures', policyClassifierTestCases()
    .filter((testCase) => testCase.audit)
    .every((testCase) => auditRelevantFiles([testCase.file]).includes(testCase.file)));
  add('verification classifier includes policy-owned verification fixtures', verificationRelevantFiles([verificationFixture]).includes(verificationFixture));
  add('audit evidence mirrors policy-owned verification classes', policyProblems().every((problem) => !problem.includes('auditMirrorsVerificationRules'))
    && requiredEvidenceClassesForFiles([auditFixture], 'audit').length > 0
    && (!designCapabilityActive() || requiredEvidenceClassesForFiles([designFixture], 'audit').length > 0));
  add('visual zoo capture asserts themed parity showcases', !designCapabilityActive() || Array.isArray(POLICY.design?.zooVisualCapture?.interactiveShowcases?.['themed-empty-state']?.expectedTexts)
  ...
  add('curated workflow research reports are explicit guide documents', !(POLICY.guide?.sourceDirectories || []).some((entry) => String(typeof entry === 'string' ? entry : entry.path).includes('.codex/workflow/research')));
  ...
  add('workflow adapter policy rejects missing canonical source', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-adapter-policy-'));
    const policy = {
      adapters: {
  ...
  add('workflow adapter check catches exact-file drift', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-adapter-drift-'));
    ensureDir(join(dir, 'adapters'));
    writeFileSync(join(dir, 'adapters', 'source.txt'), 'source\n');
  ...
  add('workflow adapter sync installs and refuses unforced drift', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-adapter-sync-'));
    ensureDir(join(dir, 'adapters'));
    writeFileSync(join(dir, 'adapters', 'source.txt'), 'source\n');
  ...
  add('workflow adapter uninstall removes clean exact files', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-adapter-uninstall-'));
    ensureDir(join(dir, 'adapters'));
    writeFileSync(join(dir, 'adapters', 'source.txt'), 'source\n');
  ...
  add('workflow adapter package scripts sync and uninstall clean managed fields', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-adapter-package-'));
    const policy = {
      gates: { packageScripts: { 'workflow:status': 'node .codex/scripts/example.mjs status' } },
    };
  ...
  add('workflow health surfaces adapter diagnostics', (() => {
    const scriptText = readFileSync(KERNEL_SCRIPT, 'utf8');
  ...
  add('workflow policy rejects package script bypasses', (() => {
    const scripts = { ...packageWorkflowScripts(), 'workflow:release-gate': `node ${WORKFLOW_WRAPPER_REL} validate` };
  ...
  add('workflow policy rejects CI command bypasses', !ciWorkflowCapabilityActive() || ciWorkflowContractProblems({
    text: readText(ciWorkflowPath()).replace('npm run workflow:release-gate', 'npm run workflow:status'),
  }).some((problem) => problem.includes('ci-release-gate')));
  add('workflow policy ignores commented CI command decoys', !ciWorkflowCapabilityActive() || ciWorkflowContractProblems({
  ...
  add('public guide uses deployment policy visual zoo URL', !deploymentCapabilityActive() || PUBLIC_ZOO_GUIDE_URL === POLICY.deployment?.visualZooGuideUrl);
  ...
  add('deployment guide artifact metadata rejects per-file drift', !deploymentCapabilityActive() || (() => {
  ...
  add('deployment target policy rejects unrelated hosts', !deploymentCapabilityActive() || deploymentTargetProblems('https://example.invalid/workflow/', 'fixture target').some((problem) => problem.includes('outside allowed')));
  ...
  add('work intake rejects patch without work slice for current substantive hash', intakeProblemsForState({
    patches: [{ id: 'PATCH-test', worktreeHash: 'hash', files: [substantiveFixture] }],
  ...
  }, { branch: { hash: 'branch-hash', files: [substantiveFixture] } }).some((problem) => problem.includes('deployments record DEPLOY-unlinked') && problem.includes('no workSliceIds')));
  add('work intake rejects done slice without evidence', intakeProblemsForState({
    ...
    workSlices: [{ id: 'WORK-SLICE-done', status: 'done', intentIds: ['INTENT-done'], summary: 'done slice', acceptance: 'done', files: [substantiveFixture], created: nowIso() }],
  ...
  add('work intake derives verification relevance from linked patch files', intakeProblemsForState({
    ...
    patches: [{ id: 'PATCH-derived', files: [substantiveFixture], workSliceIds: ['WORK-SLICE-derived'] }],
  ...
  add('work intake accepts done slice with linked evidence', intakeProblemsForState({
    ...
    workSlices: [{ id: 'WORK-SLICE-done-ok', status: 'done', intentIds: ['INTENT-done-ok'], summary: 'done slice', acceptance: 'done', files: [substantiveFixture], created: nowIso() }],
    patches: [{ id: 'PATCH-done-ok', files: [substantiveFixture], workSliceIds: ['WORK-SLICE-done-ok'] }],
  ...
  add('work intake rejects branch release with active linked slice', intakeProblemsForState({
    ...
    patches: [{ id: 'PATCH-branch', branchHash: 'branch-hash', files: [substantiveFixture], workSliceIds: ['WORK-SLICE-branch'] }],
  }, { branch: { hash: 'branch-hash', files: [substantiveFixture] } }).some((problem) => problem.includes('is not closed')));
  add('work intake accepts branch release with closed linked slice and proof', intakeProblemsForState({
    ...
    patches: [{ id: 'PATCH-branch-ok', branchHash: 'branch-hash', files: [substantiveFixture], workSliceIds: ['WORK-SLICE-branch-ok'] }],
  ...
  }, { branch: { hash: 'branch-hash', files: [substantiveFixture] } }).length === 0);
  ...
  add('guide feed hash survives self-referential deployment record after guide generation', (() => {
    ...
    const rootId = currentModel.latestSlices[0]?.rootId || currentModel.latestSlices[0]?.id || 'WORK-SLICE-SELF-TEST';
    const id = `DEPLOYMENT-SELF-TEST-${process.pid}`;
    const path = join(RECORDS, 'deployments', `${id}.md`);
    const rel = relative(ROOT, path).replaceAll('\\', '/');
    const text = [
      '---',
      `schema: "${recordSchema('deployments')}"`,
      `id: "${id}"`,
      `created: "${nowIso()}"`,
  ...
  add('visual zoo source hash is content based', !designCapabilityActive() || /^[a-f0-9]{24}$/.test(zooVisualSourceHash()));
  add('design guide paths are policy-owned, not script literals', !designCapabilityActive() || (() => {
    const scriptText = readFileSync(KERNEL_SCRIPT, 'utf8');
    const registryLiteral = DESIGN_REGISTRY_PATH;
    const themeDirLiteral = DESIGN_THEME_DIRECTORY_PATH;
  ...
  add('live design zoo validator consumes policy-owned route and showcase details', !designCapabilityActive() || (() => {
    const validatorText = readText('.codex/scripts/validate-design-zoo.mjs');
    const capture = POLICY.design?.zooVisualCapture || {};
    const toastShowcase = capture.interactiveShowcases?.toast || {};
    const forbiddenProjectLiterals = [
      `selectOption('${capture.defaultTheme || ''}')`,
      `${POLICY.design?.designRoute || ''}/toast`,
      `name: '${toastShowcase.buttonName || ''}'`,
      toastShowcase.expectedText || '',
    ].filter((value) => value && value !== "selectOption('')");
    return validatorText.includes("requiredPolicyString(WORKFLOW, 'design', 'designRoute')")
      ...
      && forbiddenProjectLiterals.every((literal) => !validatorText.includes(literal));
  })());
  add('visual zoo public manifest strips local capture URLs and paths', !designCapabilityActive() || (() => {
  ...
  add('hook config keeps hooks as workflow-kernel triggers', hookConfigProblems().filter((problem) => problem.includes(WORKFLOW_WRAPPER_REL)).length === 0);
  add('hook config validates agent config wiring', configAgentProblems().length === 0);
  add('hook config rejects missing agent config files', (() => {
    const configText = readText('.codex/config.toml');
    const agentConfig = [...configText.matchAll(/config_file\s*=\s*"([^"]+)"/g)][0]?.[1];
    if (!agentConfig) return true;
    return configAgentProblems(configText.replace(`config_file = "${agentConfig}"`, 'config_file = "agents/missing.toml"')).some((problem) => problem.includes('missing.toml'));
  })());
  ...
  add('hook dispatcher has no project wrapper literal fallback', !readText(profileProjectRel('hookDispatcher')).includes(WORKFLOW_WRAPPER_REL));
  add('public workflow URL env name is profile-owned without fallback assignment', !/const\s+PUBLIC_WORKFLOW_URL_ENV\s*=\s*[^;\n]*\|\|/.test(readText(WORKFLOW_WRAPPER_REL)));
  ...
  add('dependency audit baseline is explicit and expiring', !dependencyAuditCapabilityActive() || (() => {
  ...
  add('dependency audit baseline changes require workflow bookkeeping', !dependencyAuditCapabilityActive() || substantiveFiles(['.codex/workflow/dependency-audit-baseline.json']).includes('.codex/workflow/dependency-audit-baseline.json'));
  add('dependency audit baseline changes require verification', !dependencyAuditCapabilityActive() || verificationRelevantFiles(['.codex/workflow/dependency-audit-baseline.json']).includes('.codex/workflow/dependency-audit-baseline.json'));
  add('dependency audit baseline changes require audit', !dependencyAuditCapabilityActive() || auditRelevantFiles(['.codex/workflow/dependency-audit-baseline.json']).includes('.codex/workflow/dependency-audit-baseline.json'));
  add('dependency audit baseline rejects mismatched chain', !dependencyAuditCapabilityActive() || (() => {
  ...
  add('legacy patch schema is limited to explicit historical records', LEGACY_SCHEMA_RECORDS.size === 0
    || (LEGACY_SCHEMA_RECORDS.has('.codex/workflow/records/patches/PATCH-20260508T170713Z-design-system-toast-semantic-parity.md') && !LEGACY_SCHEMA_RECORDS.has('.codex/workflow/records/patches/PATCH-NEW-BAD-SCHEMA.md')));
  ...
  add('guide record feed hash includes handover and records', (() => {
    const feedFiles = guideRecordFeedFiles();
    const patchRecordDir = join(RECORDS, 'patches');
    const patchRecords = existsSync(patchRecordDir) ? readdirSync(patchRecordDir).filter((name) => name.endsWith('.md')) : [];
    return feedFiles.includes(profileProjectRel('currentState'))
      && (!patchRecords.length || feedFiles.some((file) => file.startsWith('.codex/workflow/records/patches/')));
  })());
  ...
  add('release closeout uses branch evidence when branch has diff', releaseCloseoutMode({ files: [WORKFLOW_WRAPPER_REL] }) === 'branch');
  ...
  add('branch evidence check rejects non-branch patch records as branch coverage', (() => {
    const branch = { base: 'base', mergeBase: 'merge-base', hash: 'branch-hash', files: [WORKFLOW_WRAPPER_REL] };
    return branchEvidenceProblemsForState(branch, {
      patches: [{ branchHash: 'branch-hash', scope: 'worktree', agent: sparkWorker }],
      reviews: [
        branchRecord({ verdict: 'pass', kind: 'general' }),
        branchRecord({ verdict: 'pass', kind: 'workflow' }),
  ...
  add('branch evidence check requires integrated review for delegated patch evidence', (() => {
    ...
    return branchEvidenceProblemsForState(branch, {
      patches: [
        ...
        { branchHash: 'branch-hash', scope: 'worktree', agent: sparkWorker, routingId: 'ROUTING-test', files: ['.codex/workflow/templates/routing.md'] },
      ],
      routing: [{ id: 'ROUTING-test', route: 'spark', worker: sparkWorker, files: ['.codex/workflow/templates/routing.md'] }],
      reviews: [
        branchRecord({ verdict: 'pass', kind: 'general' }),
        branchRecord({ verdict: 'pass', kind: 'workflow' }),
      ],
  ...
  add('branch evidence check requires routing record for delegated patch evidence', (() => {
    ...
    return branchEvidenceProblemsForState(branch, {
      patches: [
        branchRecord({ agent: 'codex-lead' }),
        { branchHash: 'branch-hash', scope: 'worktree', agent: sparkWorker, files: ['.codex/workflow/templates/routing.md'] },
      ],
      routing: [],
      reviews: [
        branchRecord({ verdict: 'pass', kind: 'general' }),
  ...
  add('branch evidence check accepts delegated patch with routing and integrated review', (() => {
    ...
    return branchEvidenceProblemsForState(branch, {
      ...
      routing: [
        { id: 'ROUTING-test', route: 'spark', worker: sparkWorker, files: ['.codex/workflow/templates/routing.md'] },
        { id: 'ROUTING-closeout', completedRoutingId: 'ROUTING-test', status: 'completed', route: 'spark', worker: sparkWorker },
      ],
      reviews: [
        branchRecord({ verdict: 'pass', kind: 'general' }),
        branchRecord({ verdict: 'pass', kind: 'workflow' }),
  ...
  add('branch evidence check still requires integrated review for stale-hash delegated branch records', (() => {
    ...
    return branchEvidenceProblemsForState(branch, {
      patches: [
        ...
        { branchHash: 'old-hash', scope: 'worktree', agent: sparkWorker, routingId: 'ROUTING-test', files: ['.codex/workflow/templates/routing.md'], branchIntroduced: true },
      ],
      routing: [{ id: 'ROUTING-test', route: 'spark', worker: sparkWorker, files: ['.codex/workflow/templates/routing.md'], branchIntroduced: true }],
      reviews: [
        branchRecord({ verdict: 'pass', kind: 'general' }),
        branchRecord({ verdict: 'pass', kind: 'workflow' }),
      ],
  ...
  add('branch evidence check ignores historical delegated records that were not introduced on this branch', (() => {
    ...
    return branchEvidenceProblemsForState(branch, {
      patches: [
        branchRecord({ agent: 'codex-lead', branchIntroduced: true }),
        { branchHash: 'old-hash', scope: 'worktree', agent: sparkWorker, files: ['.codex/workflow/templates/routing.md'], branchIntroduced: false },
      ],
      routing: [],
      reviews: [
        branchRecord({ verdict: 'pass', kind: 'general' }),
  ...
  add('branch evidence check exempts legacy-schema delegated patches from new routing requirement', (() => {
    if (!LEGACY_SCHEMA_RECORDS.size) return true;
    const branch = { base: 'base', mergeBase: 'merge-base', hash: 'branch-hash', files: [WORKFLOW_WRAPPER_REL] };
    return branchEvidenceProblemsForState(branch, {
      patches: [
        branchRecord({ agent: 'codex-lead', branchIntroduced: true }),
  ...
  add('guide browser summary accepts required deterministic targets', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-guide-summary-'));
  ...
  add('visual zoo guide check rejects missing fixture slug', !designCapabilityActive() || (() => {
  ...
  add('routing check allows stale delegated route after current lead patch coverage', routingScopeProblemsForState({
    ...
    route: 'spark',
    worker: sparkWorker,
    files: [delegatedFixture],
    status: 'active',
    worktreeHash: 'old-worker-hash',
    currentHash: 'current',
  }, [WORKFLOW_WRAPPER_REL], {
  ...
  add('routing check requires preflight for delegated worker patches', routingScopeProblemsForState({}, [delegatedFixture], {
    worktreeHash: worktreeHash(),
    workers: [sparkWorker],
  }).length > 0);
  add('routing check does not require preflight for lead-only patches', routingScopeProblemsForState({}, [WORKFLOW_WRAPPER_REL], {
    worktreeHash: worktreeHash(),
    workers: [leadWorker],
  }).length === 0);
  add('active routing supplies patch worker attribution', (() => {
    const routing = activeRoutingForPatch([delegatedFixture], {
    ...
    return routing?.worker === sparkWorker && csv(routing.workSliceIds)[0] === 'WORK-SLICE-fixture';
  })());
  add('active routing does not attribute out-of-scope patch', !activeRoutingForPatch([outsideDelegatedFixture], {
    routingId: 'ROUTING-fixture',
    status: 'active',
    worker: sparkWorker,
    files: [delegatedFixture],
  ...
  add('routing check scopes linked delegated patch files instead of unrelated dirty files', routingScopeProblemsForState({
    ...
    currentHash: 'current',
    recordedAt: '2026-05-09T00:09:00.000Z',
  }, [delegatedFixture, WORKFLOW_WRAPPER_REL], {
    worktreeHash: 'current',
    routingId: 'routing-test',
    workers: [sparkWorker],
    files: [delegatedFixture],
  ...
  add('integrated review triggers for lead plus delegated worker', requiresIntegratedReview({
    worktreeHash: worktreeHash(),
    workers: [leadWorker, sparkWorker],
  }));
  add('explicit patch records reset stale worker attribution', nextPatchWorkers([sparkWorker], {
  ...
  add('deployment evidence requires app and guide deployment command classes', !deploymentCapabilityActive() || (() => {
  ...
  add('deployment evidence rejects app-only deployment proof', !deploymentCapabilityActive() || evidenceRelevanceProblems({
  ...
  add('deployment evidence rejects guide-only deployment proof', !deploymentCapabilityActive() || evidenceRelevanceProblems({
  ...
  add('deployment evidence accepts app and guide deployment command classes', !deploymentCapabilityActive() || evidenceRelevanceProblems({
    commandEvidence: [
  ...
  add('deployment target policy allows app root and guide URL', !deploymentCapabilityActive() || (deploymentTargetProblems(POLICY.deployment.publicAppUrl, 'fixture target').length === 0
  ...
  add('guide token subset includes hit targets', !designCapabilityActive() || ['--hit-sm', '--hit-md', '--hit-lg'].every((token) => productionGuideTokenCss().includes(`${token}:`)));
  ...
  add('workflow engine finds custom workflow root', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-engine-root-'));
  ...
  add('workflow engine loads selected policy names', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-engine-load-'));
  ...
  add('workflow engine loads policy names from manifest', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-engine-manifest-'));
    ensureDir(join(dir, '.codex', 'workflow', 'policy'));
    writeFileSync(join(dir, '.codex', 'workflow', 'profile.json'), '{"project":"fixture"}\n');
  ...
  add('workflow engine rejects missing policy manifest by default', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-engine-missing-manifest-'));
    ensureDir(join(dir, '.codex', 'workflow', 'policy'));
    writeFileSync(join(dir, '.codex', 'workflow', 'profile.json'), '{"project":"fixture"}\n');
    let threw = false;
    try {
  ...
  add('workflow engine rejects missing manifest policy files by default', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-engine-missing-policy-'));
    ensureDir(join(dir, '.codex', 'workflow', 'policy'));
    writeFileSync(join(dir, '.codex', 'workflow', 'profile.json'), '{"project":"fixture"}\n');
  ...
  add('workflow engine rejects malformed json', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-engine-json-'));
  ...
  add('workflow wrapper resolves repository root from workspace subdirectory', (() => {
    const script = process.argv[1] ? resolve(process.argv[1]) : WORKFLOW_WRAPPER;
    const cwd = join(ROOT, '.codex', 'workflow');
  ...
  add('public sanitizer redacts private strings', (() => {
    const forbidden = publicGuideForbiddenStrings().filter(Boolean);
    const serverPath = forbidden.find((value) => value.includes('/root/')) || '/root/project';
    const sshPrefix = forbidden.find((value) => value.includes('.ssh')) || '~/.ssh/';
    const host = forbidden.find((value) => /\d+\.\d+\.\d+\.\d+/.test(value)) || '192.0.2.1';
    const sanitized = publicSafe(`${ROOT} ${serverPath}/artifact.diff root@${host} ${sshPrefix}key token=abc`);
    return !forbidden.some((value) => sanitized.includes(value))
      && !sanitized.includes('token=abc');
  })());
  add('timed command telemetry records successful command duration', (() => {
    const dir = mkdtempSync(join(tmpdir(), 'codex-command-run-'));
  ...
  add('routing check catches Spark out-of-scope file', routingScopeProblemsForState({
    ...
    worktreeHash: 'current',
    currentHash: 'current',
  }, [outsideDelegatedFixture], {}).length > 0);
  ...
  add('routing check catches non-lead scoped worker out-of-scope file', routingScopeProblemsForState({
    ...
    worktreeHash: 'current',
    currentHash: 'current',
  }, [outsideDelegatedFixture], {}).some((problem) => problem.includes('strong-routed work changed outside')));
  add('model routing catches fallback target mismatch', !modelRoutingScenarioResult({
    ...
    testsFailed: true,
    fallbackTarget: 'lead',
    expectedFallbackTarget: strongWorker,
...
function commandValidate(args) {
  ...
  if (args['commit-gate']) {
    ...
    if (!reviewOk || !recordsOk) {
      ...
      if (!reviewOk) {
        ...
        console.error('Run a focused workflow review, then record it with:');
        console.error(`node ${WORKFLOW_WRAPPER_REL} record-review --scope worktree --kind <kind> --verdict pass --reviewer <name> --notes "<summary>"`);
      }
      process.exit(1);
    }
  }
  if (args['release-gate'] || args['deployed-gate'] || args.full) {
    ...
    const guideActive = guideCapabilityActive();
    const designActive = designCapabilityActive();
    const deploymentActive = deploymentCapabilityActive();
    const dependencyAuditActive = dependencyAuditCapabilityActive();
    const needsGuide = guideActive && guideRelevantFiles(releaseFiles).length > 0;
    const needsZooVisual = designActive && zooVisualRelevantFiles(releaseFiles).length > 0;
    ...
    const zooOk = !designActive || commandZooCheck({ quiet: true });
    ...
    const portabilityOk = commandPortabilityCheck({ quiet: true });
    ...
    if (!reviewOk || !verifyOk || !auditOk || !handoverOk || !recordsOk || !routingOk || !guideOk || !guideBrowserOk || !zooOk || !zooVisualOk || !inventoryOk || !adapterOk || !policyOk || !portabilityOk || !traceOk || !hookConfigOk || !intakeOk || !activityOk || !branchEvidenceOk || !depAuditOk || !prodZooOk || !deploymentOk || selfTestFailures.length) {
      ...
      if (!portabilityOk) {
        console.error('Release gate failed: reusable workflow system did not pass empty-project portability role-play.');
        commandPortabilityCheck({ quiet: false });
      }
      if (!traceOk) {
  ...
  if (!ok) {
    console.log(JSON.stringify({
      hookSpecificOutput: {
        ...
        permissionDecisionReason: `${workflowMessagePrefix()}: substantive changes need a passing focused review record before git commit.`,
      },
    }));
  }
}
...
function hookStop() {
  ...
  if (!recordsOk) {
    console.log(JSON.stringify({
      continue: true,
      systemMessage: `${workflowMessagePrefix()}: committed evidence records were edited. Create correction records instead of changing historical evidence.`,
    }));
    return;
  }
  const handoverOk = commandHandoverCheck({ quiet: true });
  if (!handoverOk) {
    console.log(JSON.stringify({
      continue: true,
      systemMessage: `${workflowMessagePrefix()}: current-state handover has stale/finalization wording. Run npm run workflow:handover-check before final handover.`,
    }));
    return;
  }
  const files = substantiveFiles();
  ...
  if (!ok) {
    console.log(JSON.stringify({
      continue: true,
      systemMessage: `${workflowMessagePrefix()}: substantive changes are present without a passing review record before commit or handover.`,
    }));
    return;
  }
  ...
  if (!verifyOk || !auditOk) {
    ...
    console.log(JSON.stringify({
      continue: true,
      systemMessage: `${workflowMessagePrefix()}: substantive changes have review but still need ${missing.join(' and ')} evidence before final handover/release.`,
    }));
    return;
  }
  console.log(JSON.stringify({ continue: true }));
...
if (command === 'status') commandStatus();
  ...
  const ok = commandCapabilityCheck(args);
  process.exit(ok ? 0 : 1);
} else if (command === 'portability-check') {
  const ok = commandPortabilityCheck(args);
  process.exit(ok ? 0 : 1);
...
function isDirectKernelRun() {
  return Boolean(process.argv[1]) && import.meta.url === pathToFileURL(resolve(process.argv[1])).href;
}

if (isDirectKernelRun()) await runWorkflowCli();
