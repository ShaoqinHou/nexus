...
| Work Intake and activity tracing | Required for solo-dev AI work | `policy/intake.json`, `knowledge/work-intake.md`, `templates/intent.md`, `templates/work-slice.md`, `templates/activity.md`, records under `intents/`, `work-slices/`, and `activities/` | `workflow:work-intake-check`, `workflow:activity-check`, release gate | Keep unless the target project has another durable intent/progress system. Rewrite intent kinds, activity phase kinds, gap threshold, enforcement cutoff, and guide limits. |
...
| Generated dashboard and public guide | Required workflow surface | `policy/guide.json`, `dashboard/`, guide rendering in system kernel | `workflow:guide-check`, `workflow:guide-browser-finalize`, `workflow:guide-browser-check` | Keep the capability active. Rewrite guide titles, role cards, source files, redaction policy, graph nodes, and public/private split. Generated files must be regenerated, not copied as truth. |
...
The generated dashboard/public guide is required because it is the compact human/agent navigation surface for the workflow. Design-system/Zoo/Gym and deployment validation are still active in Nexus, but they are capability-gated in the kernel. A new or empty project should keep the guide active while disabling or stubbing those optional capabilities until real project data exists.
