...
- Use `.codex/workflow/policy/files.json` `inventory.roleTaxonomy` to classify workflow paths before editing or porting them. It identifies system code, project data/policy, workflow docs, append-only evidence, generated artifacts, mutable cache/runtime telemetry, and historical research.
